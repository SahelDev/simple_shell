...
int _parse(char *line, int num_tokens, char **argv, int cmdcount)
{
	...
	bltin = check_bltin_cmd(line, ar, newline, array); /* checks for built-ins */
	if (bltin == 0) /* if not a built-in passes into exe */
		exval = exe_(line, ar, newline, array, argv, cmdcount);
	_free(line, ar, newline, array);
	return (exval); /* returns whether or not execve failed or worked */
}

/**
 * _free - function to free memory
